...
<body class="min-h-screen bg-gray-50 bg-fixed bg-bottom bg-no-repeat">
    ...
    <main class="flex flex-col justify-center p-6 pb-12">
        ...
        <div class="mx-auto mt-6 w-full max-w-md rounded-xl bg-white/80 p-6 shadow-xl backdrop-blur-xl sm:mt-10 sm:p-10">
            <form action="{{ route('movies.update', $movie) }}" method="POST" autocomplete="off">
                @csrf
                @method('PUT')
                <div class="mb-6">
                ...
                <div class="mb-6">
                    ...
                    <div class="relative mt-1 rounded-md shadow-sm">
                        ...
                        <textarea id="description" rows="4" class="w-full rounded-md pl-10 text-sm {{ $errors->has('description') ? 'border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-green-500 focus:ring-green-500 placeholder:text-gray-300' }}" placeholder="@lang('movies.description')">{{ $movie->description }}</textarea>
                        @error('description')
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3">
                            <svg class="h-5 w-5 text-red-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                                <path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zM12 8.25a.75.75 0 01.75.75v3.75a.75.75 0 01-1.5 0V9a.75.75 0 01.75-.75zm0 8.25a.75.75 0 100-1.5.75.75 0 000 1.5z" clip-rule="evenodd" />
                ...
                <div class="mb-6">
                    ...
                    <div class="relative mt-1 rounded-md shadow-sm">
                        ...
                        <select id="genre_id" name="genre_id" class="w-full rounded-md pl-10 text-sm {{ $errors->has('description') ? 'border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-green-500 focus:ring-green-500 placeholder:text-gray-300' }}">
                            <option value="">@lang('movies.genre_not_selected')</option>
                            @foreach($genres as $key => $genre)
                                <option value="{{ $key }}" @if($movie->genre_id == $key) selected @endif>{{ $genre }}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('genre_id')
                ...
                <div class="mb-6">
                    ...
                    <div class="relative mt-1 rounded-md shadow-sm">
                        ...
                        <input type="number" id="year" name="year" value="{{ $movie->year }}" required autofocus class="w-full rounded-md pl-10 text-sm {{ $errors->has('year') ? 'border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-green-500 focus:ring-green-500 placeholder:text-gray-300' }}" placeholder="John Doe" />
                        @error('year')
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3">
                            <svg class="h-5 w-5 text-red-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                                <path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zM12 8.25a.75.75 0 01.75.75v3.75a.75.75 0 01-1.5 0V9a.75.75 0 01.75-.75zm0 8.25a.75.75 0 100-1.5.75.75 0 000 1.5z" clip-rule="evenodd" />
                ...
                <div>
                    <button type="submit" class="w-full flex items-center justify-center rounded-md bg-green-600 py-2 px-4 font-semibold text-white shadow-lg transition duration-150 ease-in-out hover:bg-green-700 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">@lang('movies.edit')</button>
                </div>
            </form>
        </div>
    </main>
